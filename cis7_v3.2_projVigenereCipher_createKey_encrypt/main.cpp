 ...
 * Purpose:  CIS 7 Final project Case 3: Vigenere Cipher v3.2_createKey_encrypt
 * 
 v3.2_createKey_encrypt:
 ...
 * Loops through the plaintext's length, and applies the Encrypt formula by 
...
void getEncrypt(char azChar[]){
    ...
    int shift = 0, encryptVal = 0, pTCharVal = 0, count = 0;
    ...
    for (int i = 0; i < pTSize; i++) {
        ...
        //cout << pTextStr[i] << " = " << setw(2) << pTCharVal << " + "
                //<< setw(2) << shift << " shift = " << setw(2) << encryptArr[i]
                //<< " == " << setw(2) << azChar[encryptVal] << endl;

    }
    cout << endl << "\nPlaintext successfully encrypted:";
    prnt3Strs(pTextStr, key, encryptStr);
}
